...
class DownController extends CommonController {
	...
	public function index() {
		...
		//获取文摘列表
		$data['down'] = $Down->getList('down',$cids,$p);
		//防止空页被访问
		if(empty($data['down']['data']) && $p > 0){
			$this->redirect('Down/index',array('cid'=>$cid));
		}
		//查询分类列表
		$data['categorydown'] = $Categorydown->getList();
		$data['cid'] = $cid;
		$data['p'] = $p;
		$this->assign($data);
		$this->display();
	...
	public function add(){
		...
		if(IS_POST){
			//创建数据对象
			if(!$Down->create(null,1)){
				$this->error('添加文摘失败：'.$Down->getError());
			}
			//处理特殊字段
			$Down->category_id = $cid; //文摘分类

			//预览图
			if(!$info) {
				$Down->thumb = '';
			} else {
				$Down->thumb = $info['thumb']['url'];
			}

			$Down->content = I('post.content','','htmlpurifier'); //资源描述（富文本过滤）
			...
			//添加文摘成功
			if(isset($_POST['return'])) $this->redirect('Down/index');
	...
	public function edit(){
		...
		$Categorydown = D('Categorydown');
		$Down = D('Down');
		$Down->thumb = I('post.url');
		 //准备where条件
		$where = array('id'=>$id,'recycle'=>'no');
		if(IS_POST){
			//创建数据对象
			if(!$Down->create(null,2)){
				$this->error('修改文摘失败：'.$Down->getError());
			}
			//处理特殊字段
			$Down->category_id = $cid;    //保存文摘分类
			$Down->content = I('post.content','','htmlpurifier'); //文摘描述（富文本过滤）
			if($info) {
				$Down->thumb = $info['thumb']['url'];  //上传成功，保存文件路径
			}

			//保存到数据库
			if(false === $Down->where($where)->save()){
				$this->error('修改文摘失败！');
			}
			//修改文摘成功
			if(isset($_POST['return'])) $this->redirect('Down/index',array('cid'=>$cid,'p'=>$p));
			$this->assign('success',true);
		}
		//查询文摘数据
		$data['down'] = $Down->getDown($where);
		if(!$data['down']){
			$this->error('修改失败：资源不存在。');
		}
		//查询分类列表
		$data['categorydown'] = $Categorydown->getList();
		$data['cid'] = $cid;
	...
	public function del() {
		...
		//生成跳转地址
		$jump = U('Down/index',array('cid'=>$cid,'p'=>$p));
		//实例化模型
		$Down = M('Down');
		//检查表单令牌
		if(!$Down->autoCheckToken($_POST)){
			$this->error('表单已过期，请重新提交',$jump);
		}
		//将文摘放入回收站
		if(false === $Down->where(array('id'=>$id))->save(array('recycle'=>'yes'))){
	...
	public function change(){
		...
		//执行操作
		if(false === $Down->where(array('id'=>$id,'recycle'=>'no'))->save(array($field=>$status))){
