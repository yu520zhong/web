...
public function buy(){
	...
	//获取收件人信息（收件人，收件地址，手机）
	$data['address'] = M('User')->field('consignee,address,phone')
	...
	$Order->commit();    //提交事务
	//生成订单成功，删除购物车中的记录
	M('Shopcart')->where(array('goods_id'=>array('IN',array_keys($buy)),
	'user_id'=>$uid))->delete();
...
public function index(){
	//取出订单列表
	$data = M('Order')->where(array('user_id'=>$this->userinfo['id'],
