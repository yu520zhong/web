...
class EssayModel extends Model {
	//表单字段过滤
	protected $insertFields = 'sn,name,desc,on_sale,recommend,author,keywords,source,url,links';
	protected $updateFields = 'sn,name,desc,on_sale,recommend,author,keywords,source,url,links';
	//自动验证
	 ...
	 * @return array 查询结果
	 */
	public function getList($type='essay',$cids=0,$p=0){
		//准备查询条件
		$order = 'a.id desc';        //排序条件
		$field = 'c.name as categoryessay_name,a.category_id,a.id,a.name,a.on_sale,a.author,a.desc,a.keywords,a.recommend,a.source,a.url,a.links,a.add_time';
		if($type=='essay'){          //商品列表页取数据时
		...
		//查询数据
		$data = $this->alias('a')->join('__CATEGORYESSAY__ AS c ON c.id=a.category_id','LEFT')->field($field)
	...
	public function getGoods($where){
		//定义需要的字段
		$field = 'id,category_id,sn,name,thumb,on_sale,recommend,desc,recycle,content,add_time,author,keywords,source,url,links';
