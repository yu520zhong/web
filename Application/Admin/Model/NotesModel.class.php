...
class NotesModel extends Model {
	//表单字段过滤
	protected $insertFields = 'sn,name,desc,on_sale,recommend,code,keywords,links';
	protected $updateFields = 'sn,name,desc,on_sale,recommend,code,keywords,links';
	//自动验证
	protected $_validate = array(
		array('name','1,100','博客名称不合法（1-100个字符）',self::MUST_VALIDATE,'length'),
		array('sn','/^[0-9A-Za-z]{1,10}$/','博客编号格式不正确（1-10个字符）',self::MUST_VALIDATE),
		...
		array('desc','1,500','摘要长度不正确（1-500个字符）',self::MUST_VALIDATE,'length'),
		array('code','1,39999','演示代码长度不合法（1-39999个字符）',self::VALUE_VALIDATE,'length'),
	 ...
	 * @return array 查询结果
	 */
	public function getList($type='notes',$cids=0,$p=0){
		//准备查询条件
		$order = 'n.id desc';        //排序条件
		$field = 'c.name as categorynote_name,n.category_id,n.id,n.name,n.desc,n.keywords,n.on_sale,n.recommend,n.add_time,n.code,n.links';
		if($type=='notes'){          //博客列表页取数据时
		...
		if($cids == 0){      //查找未分类的商品
			$where['n.category_id'] = 0;
		}elseif($cids > 0){  //查找分类ID数组
			$where['n.category_id'] = array('in',$cids);
		}
		//准备分页查询
		$pagesize = C('USER_CONFIG.pagesize');              //每页显示博客数
		$count = $this->alias('n')->where($where)->count(); //获取符合条件的商品总数
		$Page = new \Think\Page($count,$pagesize);          //实例化分页类
		$this->_customPage($Page);                          //定制分页类样式
		//查询数据
		$data = $this->alias('n')->join('__CATEGORYNOTE__ AS c ON c.id=n.category_id','LEFT')->field($field)
	...
	public function getGoods($where){
		//定义需要的字段
		$field = 'id,category_id,sn,name,thumb,on_sale,recommend,desc,keywords,content,add_time,code,links';
