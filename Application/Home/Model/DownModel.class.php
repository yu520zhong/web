...
class DownModel extends Model {
	...
	public function getList($cids=0,$p=0) {
		$field = 'category_id,id,name,desc,add_time,author,content,keywords,thumb,url,click';
		//查找分类ID数据
		if($cids > 0) {
	...
	public function postList($cids=0,$p=0) {
		$field = 'c.name as category_name,a.category_id,a.id,a.name,a.desc,a.thumb,a.sn,a.add_time,a.author,a.content,a.keywords,a.url,a.click';
		//查找分类ID数据
		if($cids > 0) {
		...
		$this->_customPage($Page);
		//查询文章数据
		$data = $this->alias('a')->join('__CATEGORYDOWN__ AS c ON c.id=a.category_id','LEFT')->field($field)->where($where)
	...
	public function getSearch($p=0,$keyword){
		...
		$field = 'id,name,thumb,author,add_time,desc,content,keywords,click,links';
	...
	/*public function postSearch($keyword){
		...
		$list4 = M ('Down')->field($field)->where($map)->select();
	...
	public function getGoods($where){
		//定义需要的字段
		$field = 'id,category_id,sn,name,thumb,author,add_time,desc,content,keywords,url,click';
	...
	public function getRecommend(){
		//准备查询条件
		$order = 'a.id asc';
		$field = 'id,name,thumb,author,add_time,click';
