...
class NotesModel extends Model {
	...
	public function getList($cids=0,$p=0){
		//准备查询条件
		$field = 'category_id,id,name,desc,thumb,add_time,click';
		//where条件
		...
		//准备分页查询
		$pagesize = C('USER_CONFIG.goodsize');        //每页显示博客数
		$count = $this->where($where)->count();       //获取符合条件的博客总数
	...
	public function postList($cids=0,$p=0) {
		$field = 'c.name as category_name,a.category_id,a.id,a.name,a.desc,a.thumb,a.add_time,a.content,a.click';
		//查找分类ID数据
		if($cids > 0) {
		...
		$this->_customPage($Page);
		//查询博客数据
		$data = $this->alias('a')->join('__CATEGORYNOTE__ AS c ON c.id=a.category_id','LEFT')->field($field)->where($where)
	...
	public function showList($cids=0,$p=0){
		//准备查询条件
		$order = 'n.id desc';
		$field = 'c.name as category_name,n.category_id,n.id,n.name,n.desc,n.thumb,n.sn,n.add_time,n.content,n.code,n.click';
		// $where = "category_id IN(1,2,3)";
		//where条件
		$where = array('recycle' => 'no','on_sale' => 'yes');
		//准备分页查询
		$pagesize = C('USER_CONFIG.pagesize');        //每页显示博客数
		$count = $this->alias('n')->where($where)->count();       //获取符合条件的博客总数
		$Page = new \Think\Page($count,$pagesize);    //实例化分页类
		$this->_customPage($Page);                    //定制分页类样式
		//查询博客数据
		$data = $this->alias('n')->join('__CATEGORYNOTE__ AS c ON c.id=n.category_id','LEFT')->field($field)->where($where)
		->order($order)->page($p,$pagesize)->select();
		//返回结果
		return array(
			'data' => $data,              //博客列表数组
	...
	public function getGoods($where){
		//定义需要的字段
		$field = 'id,category_id,sn,name,thumb,add_time,desc,content,code,click';
	...
	public function getRecommend(){
		$order = 'id desc';
		//查询被推荐的博客
		$field = 'id,name,add_time,thumb,click';
		$where = array('recycle' => 'no','on_sale'=>'yes','recommend'=>'yes');
		return $this->field($field)->order($order)->where($where)->limit(9)->select();
	}
