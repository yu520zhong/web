...
<body class="fixed-sidebar full-height-layout gray-bg" style="overflow:hidden">
                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                            <div class="top-button">
                                <h5>选择博客分类：</h5><select id="category" class="form-control">
                                    <option value="-1" >全部</option>
                                    <option value="0" <?php if(($cid) == "0"): ?>selected<?php endif; ?>>未分类</option>
                                    <?php if(is_array($categorynote)): foreach($categorynote as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(($v["id"]) == $cid): ?>selected<?php endif; ?>><?php echo str_repeat('— ',$v['level']); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
                                </select>
                                <div><h4>
                                <a href="<?php echo U('Notes/add',array('cid'=>$cid));?>" class="light">添加博客</a>
                                <a href="<?php echo U('Categorynote/add');?>">添加分类</a>
                                </h4></div>
                            </div>
                            <div class="list full">
                                <table class="table">
                                    <tr><th>ID</th><th class="t1">博客分类</th><th>博客名称</th><th >发布日期</th><th >上架</th><th>推荐</th><th>操作</th></tr>
                                    <?php if(is_array($notes["data"])): foreach($notes["data"] as $key=>$v): ?><tr>
                                        <td><?php echo ($v["id"]); ?></td>
                                        <td class="t1">
                                            <?php if(empty($v["category_id"])): ?><a href="<?php echo U('Notes/index','cid=0');?>">未分类</a>
                                            <?php else: ?>
                                                <a href="<?php echo U('Notes/index',array('cid'=>$v['category_id']));?>"><?php echo ($v["categorynote_name"]); ?></a><?php endif; ?>
                                        </td>
                                        <td><?php echo ($v["name"]); ?></td><td><?php echo ($v["add_time"]); ?></td>
                                        ...
                                        <a href="<?php echo U('Notes/edit',array('id'=>$v['id'],'cid'=>$v['category_id'],'p'=>$p));?>">修改</a>　<a href="#" class="act-del" data-id="<?php echo ($v["id"]); ?>">删除</a></td></tr><?php endforeach; endif; ?>
                                </table>
                            </div>
                            <div class="pagelist"><?php echo ($notes["pagelist"]); ?></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $("#category").change(function(){
            var url = "<?php echo U('Notes/index',array('cid'=>'_ID_'));?>";
        ...
        function change_status(obj,field){
            ...
            $("#form").attr("action","<?php echo U('Notes/change',array('p'=>$p,'cid'=>$cid));?>").submit();
        }
        //快捷操作-推荐
        ...
        $(".act-del").click(function(){
            if(confirm('确定要删除吗？')){
                ...
                $("#form").attr("action","<?php echo U('Notes/del',array('p'=>$p,'cid'=>$cid));?>").submit();
            }
        });
    </script>
</body>
