            ...
            <div class="col-md-8">
                <section class="article">
                    <ul class="filter">
                        ...
                        <?php if(!empty($categoryart["parent"])): if(is_array($categoryart["parent"])): $i = 0; $__LIST__ = $categoryart["parent"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li>
                                    <p class="cateline">
                                    ...
                                    <?php if(is_array($v)): foreach($v as $key=>$vv): ?><a href="<?php echo blogmkFilterURL('cid',$vv['id']);?>" class="cid-<?php echo ($vv["id"]); ?>" ><?php echo ($vv["name"]); ?></a><?php endforeach; endif; ?></li>
                                </li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                    </ul>
                    <?php if(empty($articles["data"])): ?><p class="no_find">很抱歉，没有找到您需要的文摘!</p><?php else: ?>
                        <?php if(is_array($articles["data"])): foreach($articles["data"] as $key=>$v): ?><article class="excerpt excerpt-text">
                                <div class="row">
...
<script>
    ...
    //导航条选中效果
    //筛选列表，分类的当前选中效果
    <?php if(is_array($categoryart["pids"])): foreach($categoryart["pids"] as $key=>$v): ?>$(".cid-<?php echo ($v); ?>").addClass("curr");<?php endforeach; endif; ?>
//文章排序的选中效果
