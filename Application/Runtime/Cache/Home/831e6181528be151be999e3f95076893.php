...
<meta name="keywords" content="<?php echo ($notes["keywords"]); ?>">
<meta name="description" content="<?php echo ($notes["desc"]); ?>">
...
<div class="focusbox">
    <div class="container">
        <h1 class="focusbox-title">我在路上</h1>
        <div class="focusbox-text">行在路上，品味生活，我的人生在路上！</div>
    </div>
</div>
<!--container start-->
<div class="content">
        ...
        <div class="col-md-8 single">
            <div class="blog-detail">
                <header class="detail-head" >
                    ...
                    <div class="meta">
                        <span class="item"><?php echo ($notes["add_time"]); ?></span>
                        <span class="item">阅读量：<?php echo ($notes["click"]); ?></span>
                        <span class="item">评论量：<a href="#SOHUCS"><span id="changyan_count_unit"></span></a></span>
                    </div>
                </header>
                <div class="detail-desc"><strong>摘要：</strong><?php echo ($notes["desc"]); ?></div>
                <article class="blog-content article-content">
                    <?php if(!empty($notes["thumb"])): ?><p class="text-center">
                            <img class="alignnone size-full wp-image-74" alt="<?php echo ($notes["name"]); ?>" src="<?php echo ($notes["thumb"]); ?>">
                        </p>
                        <?php else: endif; ?>
                     <?php echo ($notes["content"]); ?>
                     <?php if(!empty($notes["code"])): ?><p>
                            <textarea name="code" id="code" class="hide"><?php echo ($notes["code"]); ?></textarea>
                            <pre><?php echo ($notes["code"]); ?></pre>
                            <button class="btn btn-sm btn-default mt15" onclick="Preview(code)">运行代码</button>
                        </p>
                        <?php else: endif; ?>
                </article>
                <p class="post-copyright">本文属原创，转载请注明出处：<?php if(is_array($path)): foreach($path as $key=>$v): ?>&nbsp;
                                <a href="<?php echo U('Notes/index',array('cid'=>$v['id']));?>">
                                    <?php echo ($v["name"]); ?>
                                </a>
                                &nbsp;&gt;<?php endforeach; endif; ?>
                            &nbsp;<?php echo ($notes["name"]); ?></p>
                <div class="share-single"><div class="action-share bdsharebuttonbox bdshare-button-style0-16" data-bd-bind="1474380922803">
                    分享到：
                 ...
                 <ul class="pager clearfix">
                        <?php if(empty($prev)): else: ?>
                            <li class="previous">
                                <a title="<?php echo ($prev["name"]); ?>" href="<?php echo U('Notes/notes',array('id'=>$prev['id']));?>" data-toggle="tooltip" data-original-title="<?php echo ($prev["name"]); ?>">上一篇</a>
                            </li><?php endif; ?>
                        <?php if(empty($next)): else: ?>
                            <li class="next">
                                <a title="<?php echo ($next["name"]); ?>" href="<?php echo U('Notes/notes',array('id'=>$next['id']));?>" data-toggle="tooltip" data-original-title="<?php echo ($next["name"]); ?>">下一篇</a>
                            </li><?php endif; ?>
                    </ul>
                <div class="relates">
                    <h3>推荐相关：</h3>
                    <ul>
                        <?php if(is_array($latest["data"])): foreach($latest["data"] as $key=>$v): ?><li>
                                <a href="<?php echo U('Notes/notes',array('id'=>$v['id']));?>"><?php echo ($v["name"]); ?></a>
            ...
            <!-- 畅言评论框 start -->
                <div id="SOHUCS" sid="004<?php echo ($notes["id"]); ?>"></div>
            <!-- 畅言评论框 end -->
            </div>
        </div>
