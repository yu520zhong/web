...
<meta name="description" content="觅友——有朋自远方来，分享自己网站开发学习以及工作和生活中的点点滴滴，希望通过此网站，与大家一起交流、成长、进步！">
...
<div class="focusbox">
    <div class="container"><h1 class="focusbox-title">我在路上</h1>
    ...
    <div class="row">
        <div class="col-md-8">
            <section class="article">
                <ul id="filters" class="list-unstyled news-filter clearfix">
                    <?php if(!empty($categorynote["parent"])): if(is_array($categorynote["parent"])): $i = 0; $__LIST__ = $categorynote["parent"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if(is_array($v)): foreach($v as $key=>$vv): ?><li>
                                <a href="<?php echo notemkFilterURL('cid',$vv['id']);?>" class="cid-<?php echo ($vv["id"]); ?>" ><?php echo ($vv["name"]); ?></a>
                                </li><?php endforeach; endif; endforeach; endif; else: echo "" ;endif; endif; ?>
                </ul>
                <?php if(empty($notes["data"])): ?><p class="no_find">很抱歉，没有找到您需要的文摘!</p><?php else: ?>
                    <?php if(is_array($notes["data"])): foreach($notes["data"] as $key=>$v): ?><article class="excerpt excerpt-text">
                            <div class="row">
                                <?php if(!empty($v["thumb"])): ?><div class="col-md-4">
                                        <a href="<?php echo U('Notes/notes',array('id'=>$v['id']));?>" class="nail">
                                ...
                                <header class="col-md-8">
                                    <div class="excerpt-content">
                                        <h2 class="title">
                                            <a href="<?php echo U('Notes/notes',array('id'=>$v['id']));?>" target="_blank" title="<?php echo ($v["name"]); ?>"><?php echo ($v["name"]); ?></a>
                                        </h2>
            ...
            <nav class="text-center">
                <div class="pagination"><?php echo ($notes["pagelist"]); ?></div>
            </nav>
        </div>
                ...
                <ul>
                    <?php if(is_array($recommend)): foreach($recommend as $key=>$v): ?><li class="widget-post-item">
                            <a target="_blank" href="<?php echo U('Notes/notes',array('id'=>$v['id']));?>">
                                <div class="img-box">
                                    <?php if(!empty($v["thumb"])): ?><img class="thumb" alt="<?php echo ($v["name"]); ?>" src="<?php echo ($v["thumb"]); ?>" >
                                        <?php else: endif; ?>
                                </div>
                                <div class="info-box">
...
<script>
    ...
    //导航条选中效果
    //筛选列表，分类的当前选中效果
    <?php if(is_array($categorynote["pids"])): foreach($categorynote["pids"] as $key=>$v): ?>$(".cid-<?php echo ($v); ?>").addClass("curr");<?php endforeach; endif; ?>
//文章排序的选中效果
